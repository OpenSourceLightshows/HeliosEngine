...
void stm8_init_gpio(void)
{
  ...
  // Based on schematic and PWM requirements:
  // RED=PD3 (TIM2_CH2), GREEN=PC3 (TIM1_CH3), BLUE=PB4 (TIM1_CH1N)
  ...
  // LED G - PC3 (TIM1_CH3)
  PC_DDR |= (1 << 3);  // Set as output
  PC_CR1 |= (1 << 3);  // Push-pull mode
  PC_CR2 |= (1 << 3);  // Fast mode

  // LED B - PB4 (TIM1_CH1N - complementary output)
  PB_DDR |= (1 << 4);  // Set as output
  PB_CR1 |= (1 << 4);  // Push-pull mode
  PB_CR2 |= (1 << 4);  // Fast mode

  // Configure button pin as input with pull-up
  // Button - PD5 (active HIGH with internal pull-up)
...
{
  // Configure Timer 2 for PWM on Red LED
  // RED LED on PD3 (TIM2_CH2)
  ...
  // Configure PWM Mode 1 for channel 2
  TIM2_CCMR2 = 0x60; // PWM mode 1 on channel 2 (RED - PD3)
  ...
  // Configure Timer 1 for PWM on Green and Blue LEDs
  // GREEN LED on PC3 (TIM1_CH3)
  // BLUE LED on PB4 (TIM1_CH1N - complementary output)
  ...
  // Configure PWM Mode 1 for channel 1 (for complementary output on PB4)
  TIM1_CCMR1 = 0x60; // PWM mode 1 on channel 1

  // Configure PWM Mode 1 for channel 3 (for GREEN on PC3)
  TIM1_CCMR3 = 0x60; // PWM mode 1 on channel 3

  // Enable complementary output on channel 1N (PB4 - BLUE)
  TIM1_CCER1 = 0x04; // CC1NE (complementary output enable)

  // Enable output on channel 3 (PC3 - GREEN)
...
void stm8_init_interrupts(void)
{
  ...
  EXTI_CR2 = (EXTI_CR2 & 0x3F) | (0x03 << 6); // Rising and falling edges
