...
- **Pattern**: Defines LED behavior (blinking, morphing, dashing) with timing parameters
...
- **Timer**: Tick-based timing system (1000 ticks/second)
...
- Pattern parameters (7 bytes): on_dur, off_dur, gap_dur, dash_dur, group_size, blend_speed, fade_dur
- Colorset: up to 6 RGB colors (18 bytes) + count (1 byte)
- Pattern flags (1 byte)
- CRC (1 byte)
Total: 28 bytes per slot = 168 bytes for 6 slots
...
- `blend_speed`: Morph/blend transition speed (0-20)
- `fade_dur`: Fade timing for morphing patterns
