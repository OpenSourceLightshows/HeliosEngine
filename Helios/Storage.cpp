...
{
  // Unlock EEPROM
  *(volatile uint8_t *)0x50C3 = 0xAE;
  *(volatile uint8_t *)0x50C3 = 0x56;
  while (!(*(volatile uint8_t *)0x50C2 & 0x08));

  *(volatile uint8_t *)(STM8_EEPROM_BASE + address) = data;

  while (!(*(volatile uint8_t *)0x50C2 & 0x04));
  *(volatile uint8_t *)0x50C2 &= ~0x08;
}
#endif
#endif
